...
(async () => {
  ...
  await page.route('**/*',async route=>{
    ...
    if(url.pathname==='/api/ops/merch') {
      const source=url.searchParams.get('source'); observed.push(source);
      return route.fulfill({json:{orders:[website,bandcamp].filter(o=>source==='all'||o.source===source),products:[],adjustments:[],newOrders:0}});
    }
  ...
  await page.getByText('Bandcamp order 1001',{exact:false}).waitFor();
  ...
  await page.getByText('456 Corrected St',{exact:true}).waitFor();

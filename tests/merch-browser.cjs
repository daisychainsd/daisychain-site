...
(async () => {
  ...
  await page.route('**/*', async route=>{
    ...
    if(url.pathname==='/api/ops/merch') { const status=url.searchParams.get('status'); return route.fulfill({json:{orders:[order].filter(o=>status==='all'||status==='unshipped'&&o.fulfillment_status!=='shipped'||o.fulfillment_status===status),products:[product],adjustments:[],newOrders:1}}); }
  ...
  assert.equal(await page.getByRole('button',{name:'Unshipped',exact:true}).getAttribute('aria-pressed'),'true');
  assert.equal(await page.getByText('One row per shipment.',{exact:false}).count(),0);
